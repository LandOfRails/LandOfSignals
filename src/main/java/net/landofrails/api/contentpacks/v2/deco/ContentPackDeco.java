package net.landofrails.api.contentpacks.v2.deco;
...
public class ContentPackDeco {
    ...
    public static ContentPackDeco fromJson(InputStream inputStream) {
        ...
        try {
            ...
            throw new ContentPackException("Cant read ContentPackDeco: " + e.getMessage());
        }

        String json = s.toString();
        return GSON.fromJson(json, ContentPackDeco.class);
    }
    ...
    public void validate(Consumer<String> invalid, ContentPack contentPack) {
        ...
        if (joiner.length() > 2) {
            ...
            for (Map.Entry<String, ContentPackModel[]> decoModelEntry : base.entrySet()) {
                Consumer<String> decoConsumer = text -> invalid.accept(decoModelEntry.getKey() + ": [" + text + "]");
